...
function themesflat_iconpicker_type_eleganticons($icons) {
    $tmp_icon = themesflat_add_icons('icon',THEMESFLAT_LINK.'css/font-elegant.css');
...
function themesflat_available_icons($name = 'icon' ) {
    $icon_types = array ($name.'_type'=>'fontawesome',$name.'_fontawesome' => '',$name.'_openiconic' => '',$name.'_typicons' => '',$name.'_entypo' => '',$name.'_linecons' => '',$name.'_monosocial' => '',$name.'_material' => '',$name.'_simpleline' => '',$name.'_ionicons' => '',$name.'_eleganticons' => '',$name.'_icomoon' => '',$name.'_themifyicons' => '');
    return  $icon_types;
}
...
function themesflat_map_icons($name='icon',$heading_name = 'Icon') {
    return array(
            array(
                'type' => 'dropdown',
                'heading' => $heading_name.esc_html__( ' library', 'redbiz' ),
                'value' => array(
                    esc_html__( 'None', 'redbiz' ) => 'none',
                    esc_html__( 'Font Awesome', 'redbiz' ) => 'fontawesome',
                    esc_html__( 'Open Iconic', 'redbiz' ) => 'openiconic',
                    esc_html__( 'Typicons', 'redbiz' ) => 'typicons',
                    esc_html__( 'Entypo', 'redbiz' ) => 'entypo',
                    esc_html__( 'Linecons', 'redbiz' ) => 'linecons',
                    esc_html__( 'Mono Social', 'redbiz' ) => 'monosocial',
                    esc_html__( 'Material', 'redbiz' ) => 'material',
                    esc_html__( 'Font Ionicons', 'redbiz' ) => 'ionicons',
                    esc_html__( 'Font Elegant', 'redbiz' ) => 'eleganticons',
                    esc_html__( 'Font Themify Icons', 'redbiz' ) => 'themifyicons',
                    esc_html__( 'Font Icomoon Icons', 'redbiz' ) => 'icomoon',
                ),
                'std' => 'none',
                'admin_label' => true,
                'param_name' => $name.'_type',
                'description' => esc_html__( 'Select icon library.', 'redbiz' ),
            ...
            array(
                ...
                'description' => esc_html__( 'Select icon from library.', 'redbiz' ),
                'integrated_shortcode_field' => $name.'_'
            ),
            array(
                'type' => 'iconpicker',
            ...
            array(
                'type' => 'iconpicker',
                'heading' => esc_html__( 'Icon', 'redbiz' ),
            ...
            array(
                ...
                'description' => esc_html__( 'Select icon from library.', 'redbiz' ),
                'integrated_shortcode_field' => $name.'-'
            ),
...
function themesflat_custom_button_color($color = '') {
    $color = $color == '' ? themesflat_get_opt( 'scheme_color' ) : $color;
    return $color;
}
...
function themesflat_predefined_header_styles() {
    ...
    if ( empty( $styles ) ) {
        $styles = apply_filters( 'themesflat/header_styles', array(
            'header-v1' => esc_html__( 'Classic', 'redbiz' ),
            'header-v2' => esc_html__( 'Header style 02', 'redbiz' ),
            'header-v4' => esc_html__( 'Modern', 'redbiz' ),
        ) );
    }
...
function themesflat_available_social_icons() {
    $icons = apply_filters( 'themesflat_available_icons', array(
        ...
        'facebook'       => array( 'iclass' => 'fa-facebook', 'title' => 'Facebook','share_link'=>'https://www.facebook.com/sharer/sharer.php?u=' ),
...
function themesflat_menu_fallback() {
    ...
    <li>
    <a class="menu-fallback" href="' . esc_url(admin_url('nav-menus.php')) . '">' . esc_html__( 'Create a Menu', 'redbiz' ) . '</a></li></ul>';
}
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
    ...
    function themesflat_wp_title( $title, $sep ) {
        ...
        if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
            $title .= " $sep " . sprintf( esc_html__( 'Page %s', 'redbiz' ), max( $paged, $page ) );
        }
...
function themesflat_dynamic_sidebar($sidebar) {
    if ( is_active_sidebar ( $sidebar ) ) {
        ...
        if ( is_user_logged_in() ) { 
            printf( __( 'This is the %s widget area.Please go to <a href="%s">Admin</a>.', 'redbiz' ), esc_attr($sidebar), esc_url( admin_url( 'widgets.php' ) ) );
        }
    } 
}
